...
"""
def find_ORFS( min_len, max_len, sequence ) :
    uorf_list = {}
            ...
            for stop in stops[stops > start] :
                ...
                if(orf_len % 3 == 0) :
                    if start > CDS_stop:
                        ...
                        add_dic("True_Stop", stop, uorf_list)
                        add_dic("Start_Codon", current[start:start + 3], uorf_list)
                        add_dic("Start", start, uorf_list)
                        add_dic("Stop", stop, uorf_list)
                        add_dic("Type", "nonoverlapping_dORF", uorf_list)
                        break
                        ...
                        if (orf_len >= min_len and orf_len <= max_len) :
                            ...
                            if start < CDS_start :
                                if stop < CDS_start:
                                    add_dic("Start", start, uorf_list)
                                    add_dic("Stop", stop, uorf_list)
                                    add_dic("Type", "nonoverlapping_uORF", uorf_list)
                                else:
                                    add_dic("Start", start, uorf_list)
                                    add_dic("Stop", CDS_start, uorf_list)
                                    add_dic("Type", "overlapping_uORF", uorf_list)
                    break
        file_start = next_file_start
    return uorf_list
...
"""
def run_ORFs(appris_file, min_len, max_len):
    with gzip.open(appris_file, 'rt', encoding='utf-8') as f:
        sequence = (f.read())  # contains a string with the entire listing of files

    orfs = find_ORFS(min_len, max_len, sequence)
    orf_df = pd.DataFrame.from_dict(orfs)
...
- records: dataframe of transcript_df with ORF reads in tidy format
"""
def orf_reads(ribo_path, transcript_df, experiments = None):
    ...
    if experiments is None:
        experiments = r_file.experiments
    ...
    # for every experiment, run through the list of inputted gene names
    for j in experiments:  # run through each experiment
        ...
        for k in range(mmin, (mmax + 1)):
            ...
            for l in range(len(genes)):  # run through the gene names
                ...
                try:
                    ...
                    offset_sum = np.sum(offset_reads)
                    if offset_sum > 0: # Only update if >0 reads
                        exp_reads[(gene, start, stop)] += offset_sum
    ...
    exp_list = []
    for (gene, start, stop), orf_sum in exp_reads.items():
        exp_list.append({
            "Gene": gene,
            "Start": start,
            "Stop": stop,
...
def filter_orfs(df, experiments, cpm_thresh=10, perc=0.75):
    ...
    summary = (
        df2
        .groupby(
            ['Gene','Start','Stop','True_Stop', 'Start_Codon','Experiment'],
            as_index=False
        )
        .agg(CPM=('CPM','sum'))
        .pivot_table(
    ...
    keep_idxs = []
    for (_, stop), grp in summary.groupby(['Gene','True_Stop']):
...
def find_periodicity(df):
    ...
    # 2) keep only ORFs with at least one Accept==1
...
def find_chisquared(arr):
    ...
    for row in arr:
        total = sum(row)
        # Skip rows where the total is zero
        if total == 0:
...
def main() :
    ...
    appris_file = "/home1/09369/umapaul/references/appris_mouse_v2_selected.fa.gz"
    ribo_file = '/home1/09369/umapaul/references/CA1_all_replicates.ribo'
    output_file = '/home1/09369/umapaul/output/allorf_outputNEURALTEST.csv'

    allORF_df = run_ORFs(appris_file, 33, 300)
    reads_df = orf_reads(ribo_file, allORF_df, experiments = None)
